...
object SparkAPIs extends App {
  ...
  println(rdd.count()) // ~2s on camera - might vary on your PC

  val df = rdd.toDF("id")
  println(df.count()) // 33s on camera - might vary on your PC
